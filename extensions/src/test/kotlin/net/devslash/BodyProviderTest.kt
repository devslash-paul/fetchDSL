...
internal class BodyProviderTest {
  ...
  fun testWithBody() {
    val provider = getBodyProvider(
      getCall(HttpBody(null, null, mapOf("a" to listOf("b"), "c" to listOf("d")), formIdentity, null, null)),
      requestDataFromList()
    )
  ...
  fun testBodyWithReplaceableValues() {
    val provider = getBodyProvider(
      getCall(HttpBody("a=!1!&c=!2!", indexValueMapper, null, null, null, null)),
  ...
  fun testParamsWithReplacement() {
    val provider = getBodyProvider(
      getCall(HttpBody(null, null, mapOf("a" to listOf("!1!"), "c" to listOf("!2!")), formIndexed, null, null)),
      requestDataFromList(listOf("b", "d"))
    )
